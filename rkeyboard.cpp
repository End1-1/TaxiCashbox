...
void RKeyboard::setLineEdit(LineEdit *l)
{
    fLineEdit = l;
    connect(fLineEdit, SIGNAL(textEdited(QString)), this, SLOT(on_leResult_textEdited(QString)));
}

void RKeyboard::setText(const QString &text)
{
    fLineEdit->setText(text);
}
...
void RKeyboard::btnTextClicked()
{
    ...
    fLineEdit->setText(fLineEdit->text() + b->text());
    if (fShiftOn) {
        fShiftOn = false;
        setupEnglish();
    }
    fLineEdit->setCursorPosition(fLineEdit->text().length());
    emit textChanged(fLineEdit->text());
}

void RKeyboard::on_btnSpace_clicked()
{
    fLineEdit->setText(fLineEdit->text() + " ");
    emit textChanged(fLineEdit->text());
}

void RKeyboard::on_btnBackspace_clicked()
{
    fLineEdit->setText(fLineEdit->text().left(fLineEdit->text().length() - 1));
    emit textChanged(fLineEdit->text());
}
